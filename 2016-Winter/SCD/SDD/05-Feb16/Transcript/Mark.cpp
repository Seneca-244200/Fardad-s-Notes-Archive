...
namespace sict{
  ...
  Mark operator+(Mark m, int value){
 /*   Mark temp(m.totalMark() + value, m.outOf());
    return temp;*/
    return Mark(m.totalMark() + value, m.outOf());
  }
  // friend version knife in the back
  //Mark operator+(Mark m, int value){
  //  m.totalMark_ += value;
  //  return m;
  //}
