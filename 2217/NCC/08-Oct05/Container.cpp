...
namespace sdds {
   ...
   std::istream& Container::read(std::istream& input)  {
      char slash = ' ';
      input >> m_value;  // 200/400
      input >> slash;
      input >> m_capacity;
      return input;
   }
   ...
   std::ostream& operator<<(std::ostream& ostrLO, const Container& RO) {
      return RO.display(ostrLO);
   }

   std::istream& operator>>(std::istream& istrLeftOperand, Container& RightOperand) {
      return RightOperand.read(istrLeftOperand);
   }

   Container operator+(int vLO, const Container& RO) {
//      return Container(vLO + (int)RO, RO.getCapacity());
      return Container(vLO + (int)RO, ~RO);
   }

   int operator~(const Container& Op) {
      return Op.getCapacity();
   }
