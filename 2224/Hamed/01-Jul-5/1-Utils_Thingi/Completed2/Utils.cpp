...
namespace sdds {
   ...
   istream& Utils::getint(int& num, const char* prompt, istream& istr) {
   ...
   int Utils::strcmp(const char* s1, const char* s2, int len) {
      int i = 0;
   ...
   char* Utils::strcpy(char* des, const char* src, int len) {
      int i;
      for (i = 0; i< len && src[i]; i++) des[i] = src[i];
      des[i] = 0; // unlike strncpy, this function null terminates
      return des;
   }
   char* Utils::strncpy(char* des, const char* src, int len) {
      int i = 0;
      while (i < len - 1 && src[i]) {
         des[i] = src[i];
         i++;
   ...
   char* Utils::lowerstrcpy(char des[], const char source[]) {
      int i = 0, j = 0;
      for (; source[i] != 0; i++) {
