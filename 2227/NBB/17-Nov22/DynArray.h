...
// Change all the types of interest to template type
// Add the template type signature to all the clas names 
// except: The name of the class definition
...
namespace sdds {
   template <typename T>
   class DynArray {
      T* m_data{};// in all cases m_data will be null when the DynArray is instantiated
      unsigned int m_size;
   public:
   ...
   //DynArray::DynArray(unsigned int size) {
   //   m_data = new int[size];
   //   m_size = size;
   //}
   template<typename T>
