...
std::ostream& Container::display(std::ostream& os)const{
  os << "_data:" << _data;
  return os;
}
...
std::ostream& operator<<(std::ostream& os, const Container& ROP){
  return ROP.display(os);
}
std::istream& operator>>(std::istream& is, Container& ROP){
  is >> ROP._data;
  return is;
}