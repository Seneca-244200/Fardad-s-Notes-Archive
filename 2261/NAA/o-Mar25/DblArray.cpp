...
size_t DblArray::noOfPrns() const {
   return m_noOfCsvPrints;
}
...
std::ostream& DblArray::printCsv(std::ostream& ostr) const {
   size_t* csvPrnPtr = const_cast<size_t*>(&m_noOfCsvPrints);
   (*csvPrnPtr)++;// change the contents of the constant attribute using the const_cast 
                  // and a read/writable pointer to the attribute
