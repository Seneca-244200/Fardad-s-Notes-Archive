...
namespace sdds {
   ...
   Mark Mark::operator+(int value) const {
      //To Do: uncomment below and walk through the difference
      /*Mark sum(m_title, m_value + value, m_max);
      return sum;
      */
      // creating a temprary namelss Mark and return
   ...
   Mark operator+(int value, const Mark& rO) {
      return Mark((const char*)rO, rO.get()+ value,rO.max());
   }
