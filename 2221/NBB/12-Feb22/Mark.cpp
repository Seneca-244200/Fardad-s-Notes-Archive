...
namespace sdds {
   ...
   float Mark::gpa() const {
      float value; // you might get a warning since
                   // compiler does not know that you are
                   // setting the value in get.
      gpa(value);
      return value;
   }
   Mark::operator float() const {
      return gpa();
   }
   void Mark::gpa(float* valuePtr) const {
      gpa(*valuePtr);// converts the address to a reference
   }
   ...
   Mark& Mark::operator=(int markValue) {
      return set(markValue);
   }
   ...
   const char* Mark::grade( char* grade ) {
      int p = percent();
      if (p < 50) {
         strCpy(m_tempGrade, "F"); // 0
      }
      else if (p < 55) {
         strCpy(m_tempGrade, "D"); // 1
      }
      else if (p < 60) {
         strCpy(m_tempGrade, "D+"); // 1.5
      }
      else if (p < 65) {
         strCpy(m_tempGrade, "C"); // 2
      }
      else if (p < 70) {
         strCpy(m_tempGrade, "C+"); // 2.5
      }
      else if (p < 75) {
         strCpy(m_tempGrade, "B"); // 3.0
      }
      else if (p < 80) {
         strCpy(m_tempGrade, "B+"); // 3.5
      }
      else if (p < 90) {
         strCpy(m_tempGrade, "A"); // 4
      }
      else if (p <= 100) {
         strCpy(m_tempGrade, "A+"); // 4
      }
