...
namespace sdds {
   ...
   Mark& Mark::operator=(const Mark& toCopyFrom) {
      if (this != &toCopyFrom ) {  // preventing self assignment
         delete[] m_title;
         m_title = nullptr;
         set(toCopyFrom.m_title, toCopyFrom.m_value, toCopyFrom.m_max);
      }
   ...
   // queries
   ostream& Mark::display(const char* message, ostream& ostr)const {
   ...
   ostream& operator<<(ostream& LOostr, const Mark& RO) {
      return RO.display(nullptr, LOostr);
   }
