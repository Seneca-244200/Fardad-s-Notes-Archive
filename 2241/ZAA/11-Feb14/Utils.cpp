...
namespace seneca {
   ...
   bool Utils::getCstr( char*& str, istream& istr ) {
      return getCstr( str, istr, '\n' );
   }

   bool Utils::getCstr( char*& str, std::istream& istr, char delimiter ) {
      char buf[MAX_DYNAMIC_READ];
      delete[] str;
      str = nullptr;
      istr.getline( buf, MAX_DYNAMIC_READ, delimiter );
      if ( istr ) { // not failed
         str = new char[ut.strlen( buf ) + 1];
         ut.strcpy( str, buf );
      }
      return str != nullptr;
   }
